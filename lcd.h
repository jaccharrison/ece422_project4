...
#define LCD_WAIT3 6400 /* >= 100us third stage delay req'd during init */
...
#define ONE_LINE 0x00 /* One line mode */
#define TWO_LINES 0x01 /* Two line mode */

#define SMALL_FONT 0x00 /* Smaller font */
#define BIG_FONT 0x01 /* Bigger font for 1 line mode only */
...
void init_lcd(int, int); /* LCD intialization function */
void lcd_print(char); /* Prints a single character on the LCD */
void lcd_print_str(char *); /* Prints a message on the LCD */
void lcd_place_str(char *, int, int); /* Prints a message at a given position */
void lcd_clr_screen(void); /* Clears the LCD screen */
void lcd_set_cur(int, int); /* Set new cursor position */
void lcd_return_home(void); /* Reset position to (upper) left corner */
